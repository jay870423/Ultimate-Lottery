    ...
    <div className="flex-1 flex flex-col items-center justify-center relative p-4 md:p-6 w-full h-full overflow-hidden">
      ...
      <div className="absolute top-4 md:top-10 w-full text-center z-10 px-4">
        <div className="inline-block relative max-w-full">
            <div className="absolute -inset-1 bg-gradient-to-r from-neon-pink to-neon-blue rounded-lg blur opacity-75 animate-pulse-fast"></div>
      ...
      <div className="relative w-full max-w-3xl aspect-[16/9] flex items-center justify-center z-0 mt-12 md:mt-0">
        ...
        {/* Decorative Rings */}
        <div className="absolute inset-0 border-[1px] border-slate-700 rounded-full scale-[1.2] opacity-20 animate-[spin_10s_linear_infinite]"></div>
        <div className="absolute inset-0 border-[1px] border-neon-blue rounded-full scale-[1.1] opacity-20 animate-[spin_15s_linear_infinite_reverse]"></div>
        
        {/* Center Card */}
        <div className={`
           relative w-full max-w-[90vw] md:max-w-lg aspect-video bg-slate-900/80 backdrop-blur-xl border-2 rounded-2xl flex flex-col items-center justify-center shadow-[0_0_50px_rgba(0,0,0,0.5)] transition-all duration-300 overflow-hidden
           ${isDrawing ? 'border-slate-500 scale-100' : 'border-neon-pink scale-105 shadow-[0_0_50px_rgba(255,0,255,0.3)]'}
        `}>
           <div className="text-4xl md:text-8xl mb-4 opacity-10 absolute top-2 right-2 md:top-4 md:right-4 font-mono">
             {/* Background decorative ID */}
             {centerContent?.id.slice(0, 4)}
           </div>

           <div className={`px-4 text-4xl md:text-6xl lg:text-8xl font-black text-center font-sans tracking-tight break-words w-full ${isDrawing ? 'text-slate-300 blur-[1px]' : 'text-transparent bg-clip-text bg-gradient-to-br from-white to-neon-blue drop-shadow-lg'}`}>
              {centerContent?.name}
           </div>
      ...
      <div className="absolute bottom-16 md:bottom-20 z-20 w-full flex justify-center px-4">
        {remainingCount > 0 ? (
          ...
          <div className="text-xl md:text-2xl text-slate-500 font-display bg-slate-900/80 px-4 py-2 rounded">本轮抽奖已结束</div>
        )}
      </div>

      {/* Latest Winners Ticker for this prize */}
      {currentPrizeWinners.length > 0 && (
         <div className="absolute bottom-2 md:bottom-6 w-full max-w-5xl overflow-hidden pointer-events-none px-2">
            <div className="flex gap-2 md:gap-4 justify-center flex-wrap opacity-70">
               {currentPrizeWinners.map((w, i) => (
                 <div key={i} className="bg-slate-800/80 border border-slate-600 px-2 py-1 md:px-4 md:py-2 rounded-full text-xs md:text-sm text-slate-300 flex items-center gap-1 md:gap-2 animate-fadeIn whitespace-nowrap">
